...
export default function SeasonSection() {
  ...
  const handleEditSeason = (id: string | number) => {
    // Skip if this is the fallback season string ID
    if (typeof id === 'string' && isNaN(parseInt(id))) {
      console.warn('Cannot edit fallback season');
      return;
    }
  ...
  const handleSetActiveStatus = async (id: string | number, active: boolean) => {
    // Skip if this is the fallback season string ID
    if (typeof id === 'string' && isNaN(parseInt(id))) {
      console.warn('Cannot activate fallback season');
      return;
    }
    const seasonId = typeof id === 'string' ? parseInt(id) : id;
    await updateSeason(seasonId, { status: active ? 'active' : 'draft' });
  };

  // Delete a season
  const handleDeleteSeason = async (id: string | number) => {
    // Skip if this is the fallback season string ID
    if (typeof id === 'string' && isNaN(parseInt(id))) {
      console.warn('Cannot delete fallback season');
      return;
    }
    if (window.confirm('Are you sure you want to delete this season? All player data and results will be permanently lost. This cannot be undone.')) {
      const seasonId = typeof id === 'string' ? parseInt(id) : id;
  ...
  // Reset a season
  const handleResetSeason = async (id: string | number) => {
    // Skip if this is the fallback season string ID
    if (typeof id === 'string' && isNaN(parseInt(id))) {
      console.warn('Cannot reset fallback season');
      return;
    }
    if (window.confirm('Are you sure you want to reset this season? This will clear all player data and results, but keep the season structure. This cannot be undone.')) {
      const seasonId = typeof id === 'string' ? parseInt(id) : id;
      await resetSeason(seasonId);
    }
  };

  // Check if we have a valid season
  if (!currentSeason) {
    return (
      <Card className="p-4">
        <div className="text-center py-6">
          <p className="text-muted-foreground">No season available. Create one to get started.</p>
        </div>
      </Card>
    );
  }
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                {currentSeason?.name} ({currentSeason && formatSeasonDateRange(currentSeason)})
              </p>
            </div>
            <Button 
              variant="outline" 
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-2 md:mt-0">
                        ...
                        <Button 
                          ...
                          aria-label={season.isActive ? `Deactivate ${season.name}` : `Activate ${season.name}`}
                        >
                        ...
                        <Button 
                          ...
                          className="h-8 text-xs text-muted-foreground hover:text-orange-500"
                          aria-label={`Reset ${season.name}`}
                        >
                          Reset
                        </Button>
