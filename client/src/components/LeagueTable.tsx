...
export default function LeagueTable({ playerCount }: LeagueTableProps) {
  ...
  const { league, leaguePlayers, addLeaguePlayer, recordResult, removeResult, getLeagueStandings, resetLeague } = useLeague();
  ...
  const playersWithStats = useMemo(() => {
    return leaguePlayers.map(player => {
      ...
      const totalWins = player.tournamentResults?.filter(r => r.position === 1).length || 0;
      const averagePoints = totalGames > 0 ? Math.round(player.totalPoints / totalGames) : 0;
      const firstPlaces = totalWins;
      const secondPlaces = player.tournamentResults?.filter(r => r.position === 2).length || 0;
      const thirdPlaces = player.tournamentResults?.filter(r => r.position === 3).length || 0;
      const finalTableAppearances = player.tournamentResults?.filter(r => r.position && r.position <= 6).length || 0;
      const resultsWithPosition = player.tournamentResults?.filter(r => r.position) || [];
      const averagePosition = resultsWithPosition.length > 0
        ? Math.round((resultsWithPosition.reduce((s, r) => s + (r.position || 0), 0) / resultsWithPosition.length) * 10) / 10 : 0;
      const cashWinnings = player.tournamentResults?.reduce((s, r) => s + ((r as any).prizeMoney || 0), 0) || 0;
      const hits = player.tournamentResults?.reduce((s, r) => s + ((r as any).knockouts || (r as any).playersEliminatedCount || 0), 0) || 0;
      const totalInvestment = player.tournamentResults?.reduce((s, r) => {
        const buyIn = r.buyIn || (r as any).buyInAmount || 10;
        return s + buyIn + ((r as any).rebuys || 0) * ((r as any).rebuyAmount || buyIn) + ((r as any).addons || 0) * ((r as any).addonAmount || buyIn);
      }, 0) || 0;
      ...
      const recentAvg = recentGames.length > 0 ? recentGames.reduce((s: number, r: any) => s + r.points, 0) / recentGames.length : 0;
      ...
      return { ...player, totalGames, totalWins, averagePoints, firstPlaces, secondPlaces, thirdPlaces, finalTableAppearances, averagePosition, cashWinnings, hits, profit, roi, formScore, bestFinish, winRate };
    });
  }, [leaguePlayers]);

  const enabledStats = Object.entries(settings?.statsToDisplay || {}).filter(([_, v]) => v).map(([k]) => k);
  const statLabels: Record<string, string> = {
    points: 'Points', hits: 'Hits', games: 'Games', averagePoints: 'Avg Pts',
    firstPlaceFinishes: '1st', secondPlaceFinishes: '2nd', thirdPlaceFinishes: '3rd',
    cashWinnings: 'Winnings', recentForm: 'Form', bestFinish: 'Best', winRate: 'Win %',
    averagePosition: 'Avg Pos', finalTableAppearances: 'Finals', headsUpRecord: 'H2H',
    earlyExits: 'Early Out', profit: 'Profit', roi: 'ROI (%)'
  };

  const handleAddPlayer = () => {
    const name = newPlayerName.trim();
    if (name) { addLeaguePlayer(name); setNewPlayerName(''); }
  };

  const handleRecordResult = () => {
    if (!selectedPlayer || position < 1) return;
    const today = new Date().toISOString().split('T')[0];
    const player = leaguePlayers.find(p => p.id === selectedPlayer);
    if (player?.tournamentResults.some(r => r.date.split('T')[0] === today)) {
      alert('This player already has a result recorded for today.');
      return;
    }
        ...
        <div className="p-5 pt-0 border-t border-[#2a2a2a]">
          {currentSeason && (
            <div className="mb-4 pb-3 border-b border-[#2a2a2a]">
              ...
              <p className="text-sm text-muted-foreground">Season Dates: {formatSeasonDateRange(currentSeason)}</p>
              <p className="text-sm text-muted-foreground">Player Count: {(currentSeason as any).players?.length || 0} registered</p>
            </div>
          )}
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button onClick={handleAddPlayer} className="flex items-center justify-center bg-primary hover:bg-[#7722ff] text-white font-medium py-2 px-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="col-span-1">
                <div className="flex items-center">
                  ...
                  />
                  <div className="ml-2 text-muted-foreground">({calculatePoints(position, playerCount)} pts)</div>
                </div>
              </div>
              <div>
                <Button
              ...
              {(() => {
                ...
                const todayResults = leaguePlayers.flatMap(player =>
                  player.tournamentResults
                    ...
                    .map(r => ({ ...r, playerName: player.name, playerId: player.id }))
                ...
                return todayResults.length > 0 ? (
                  todayResults.map(result => (
                    <div key={result.id} className="flex items-center justify-between py-1 text-sm">
                      ...
                      <Button variant="destructive" size="sm" onClick={() => removeResult(result.playerId, result.id)} className="h-6 w-6 p-0">
                        <span className="material-icons text-xs">close</span>
                      </Button>
                    </div>
                  ))
                ) : (
                  <div className="text-center text-muted-foreground text-sm py-2">No results recorded today</div>
                );
              })()}
            </div>
          </div>
                  ...
                  <TableHeader className="bg-[#2a2a2a] sticky top-0 z-10">
                    <TableRow>
                      ...
                      {enabledStats.map(stat => (
                        <TableHead key={stat} className="text-white text-right w-16 min-w-[64px] px-2">{statLabels[stat] || stat}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {standings.length > 0 ? (
                      standings.map((player, index) => {
                        const ep = playersWithStats.find(p => p.id === player.id) || player;
                        return (
                          <TableRow key={player.id} className={index % 2 === 0 ? 'bg-[#1e1e1e]' : ''}>
                            ...
                            {enabledStats.map(stat => (
                              <TableCell key={stat} className="text-right w-16 min-w-[64px] px-2 text-sm">{getPlayerStat(ep, stat)}</TableCell>
                            ))}
                          </TableRow>
                        );
                      })
                      ...
                      <TableRow>
                        <TableCell colSpan={2 + enabledStats.length} className="text-center py-6 text-muted-foreground">No players in the league yet</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
          ...
          <div className="mt-4 flex justify-end">
            <Button variant="destructive" onClick={resetLeague} className="flex items-center justify-center gap-2 bg-destructive hover:bg-opacity-90 text-white font-medium py-2 px-4 rounded-lg">
