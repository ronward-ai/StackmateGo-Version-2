...
export function useTournamentHistory() {
  ...
  const saveCompletedTournament = useCallback((tournamentState: TournamentState) => {
    ...
    const totalPrizePool = (tournamentState.players.length * (tournamentState.prizeStructure?.buyIn || 0)) +
                          (totalRebuys * (tournamentState.prizeStructure?.rebuyAmount || 0)) +
                          (totalAddons * (tournamentState.prizeStructure?.addonAmount || 0));
  ...
  const getSeasonTournaments = useCallback((seasonId?: string) => {
