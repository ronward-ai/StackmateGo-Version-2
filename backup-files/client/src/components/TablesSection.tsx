...
export default function TablesSection({ tournament }: TablesSectionProps) {
  ...
  const seatPlayersManually = (selectedPlayers: Player[]) => {
    console.log('Manual seating requested for players:', selectedPlayers.map(p => p.name));
    console.log('MANUAL SEATING - Assigning specific table positions');
    ...
    if (playerCount <= seatsPerTable) {
      ...
      console.log(`Seating ${playerCount} players at single table (Table 1)`);
      ...
      console.log(`Distributing ${playerCount} players evenly across ${numberOfTables} tables`);
    ...
    console.log(`✅ Smart seating: ${newlySeatedPlayers.length} players assigned optimally`);
    newlySeatedPlayers.forEach(p => {
      console.log(`📍 ${p.name} -> Table ${p.tableAssignment!.tableIndex + 1}, Seat ${p.tableAssignment!.seatIndex + 1}`);
    });
    ...
    // Broadcast seating changes to participant view for database tournaments
    if (state.details?.type === 'database' && state.details?.id) {
      setTimeout(() => {
        fetch(`/api/tournaments/${state.details.id}/timer-update`, {
          method: 'POST',
          ...
          console.error('Failed to broadcast seating changes:', error);
        });
      }, 100);
    }
  };
  ...
  const handleBustOut = () => {
    ...
    // Eliminate the player and record who knocked them out (with seat info)
    eliminatePlayer(playerToBustOut.id, hitmanId, seatInfo);

    // Increment the knockouts for the hitman
  ...
  const balanceTablesRandomly = () => {
    ...
    if (targetSeat) {
      ...
      updatePlayers(updatedPlayers);
      console.log(`🎲 Randomly moved ${playerToMove.name} from Table ${balanceOptions.overloadedTable + 1} to Table ${balanceOptions.underloadedTable + 1}`);
    }
  ...
  const movePlayerToSeat = (targetTableIndex: number, targetSeatIndex: number) => {
    ...
    if (targetSeatOccupied) {
      console.log('Target seat is occupied');
      return;
    }
    ...
    console.log(`Moved ${selectedPlayerToMove.name} to Table ${targetTableIndex + 1}, Seat ${targetSeatIndex + 1}`);
  };
    ...
    <Card className="p-4 bg-gradient-to-r from-slate-800/50 to-slate-700/50 border border-slate-600/30 backdrop-blur-sm">
      ...
      >
        <h2 className="text-xl font-semibold flex items-center">
          <span className="material-icons mr-2 text-secondary">table_restaurant</span>
          Tables
        </h2>
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-2">
              <Button 
                ...
                variant="outline"
                className="flex items-center justify-center gap-1 bg-card border border-primary text-primary hover:bg-primary hover:bg-opacity-10 font-medium text-sm w-full sm:w-auto py-3 sm:py-2 min-h-[3rem] sm:min-h-auto touch-manipulation"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {Array.from({ length: Math.min(6, numberOfTables) }).map((_, tableIndex) => {
              ...
              return (
                <div 
                  key={tableIndex} 
                  className={`relative ${tableBackgrounds[tableIndex]} rounded-2xl p-5 border shadow-xl transition-all duration-300 ${
                    moveMode 
